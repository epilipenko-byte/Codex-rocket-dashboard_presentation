...
const METRIC_REJECT_TOKENS = {
  ...
  margin_pct: ['валов', 'операцион', 'ebitda', 'ebitdac', 'чист', 'чп', 'net profit', 'usd', 'руб', 'сумма', 'amount'],
  gross_margin_pct: ['маржинальн', 'операцион', 'ebitda', 'ebitdac', 'чист', 'чп', 'net profit', 'usd', 'руб', 'сумма', 'amount'],
  op_margin_pct: ['валов', 'маржинальн доход', 'чист', 'чп', 'net profit', 'ebitda', 'usd', 'руб', 'сумма', 'amount'],
  np_margin_pct: ['валов', 'маржинальн', 'операцион', 'ebitda', 'ebitdac', 'gross', 'usd', 'руб', 'сумма', 'amount'],
...
const METRIC_REQUIRED_TOKENS = {
  ...
  op_margin_pct: ['операцион', 'operating', 'ebitdac', 'ebitcac'],
...
const METRICS = {
  ...
  margin_pct: [
    ...
    'marginal income margin', 'contribution margin ratio',
    'contribution margin pct', 'contribution margin percent',
    'contribution margin', 'margin pct'
  ],
  gross_margin_pct: [
    'рентабельность по валовой прибыли', 'gross profit margin', 'gross margin'
  ],
  op_margin_pct: [
    'рентабельность по операционной прибыли', 'рентабельность по ebitcac',
    'ebitcac margin', 'operating margin'
  ],
  np_margin_pct: [
...
function doGet(e) {
  ...
  try {
    ...
    else if (action === 'get_core' || action === 'getCore') result = getCoreData();
...
function getAllData() {
  return getDataPayload({ mode: 'full' });
}

function getCoreData() {
  return getDataPayload({ mode: 'core' });
}

function getDataPayload(options) {
  ...
  const visibleReports = reports.filter(report => mode === 'core' ? shouldReadCoreReport(report) : shouldReadReport(report));
  ...
  if (mode !== 'core') out.analysis = getAiAnalysis(config);
  out.generated_at = new Date().toISOString();
  out.performance = {
    mode: mode,
    started_at: started.toISOString(),
    duration_ms: new Date().getTime() - started.getTime(),
    sources_read: contexts.length
  };
  return out;
}
...
function shouldReadCoreReport(report) {
  const rawId = norm(getAny(report, ['report_id','id','type','report_type','верхний_подвал','name','название']));
  return ['opiu','dds_common','balance'].indexOf(rawId) >= 0;
}
...
function readReport(report) {
  ...
  try {
    ...
    const sheet = findSheet(ss, sheetName, sheetRef, id);
...
function attachRawDataset(out, context) {
  ...
  const exactId = normText(context.id || '');
  ...
  if ((exactId === 'opiu' || exactId === 'pnl' ||
      idNorm.indexOf('opiu') >= 0 || idNorm.indexOf('опиу') >= 0 || idNorm.indexOf('pnl') >= 0) &&
  ...
  const isPaymentPlan = exactId === 'payment_plan' || idNorm.indexOf('плановый реестр') >= 0 || idNorm.indexOf('paymentplan') >= 0;
  const isPayment = exactId === 'payment_cal' || exactId === 'payment' ||
    idNorm.indexOf('payment') >= 0 || idNorm.indexOf('плат') >= 0 || idNorm.indexOf('календар') >= 0;
  if (isPaymentPlan) out.payment_plan_data = raw;
  else if (isPayment) out.payment_data = raw;
...
function extractMetrics(context) {
  ...
  for (let r = 0; r < data.length; r++) {
    ...
    Object.keys(METRICS).forEach(metric => {
      ...
      const candidateValues = Object.keys(valuesByKey).map(k => valuesByKey[k]);
      if (metric === 'traffic_cost' && looksLikeRatioValues(candidateValues)) return;
      if (hasUsefulValues(candidateValues)) {
...
function looksLikeRatioValues(values) {
  const nums = (values || []).map(toNum).filter(v => !isNaN(v) && Math.abs(v) > 0.000001);
  return nums.length > 0 && nums.every(v => Math.abs(v) <= 5);
}
...
function metricScoreFor(metric, label) {
  ...
  const exactBoosts = {
    op_margin_pct: ['рентабельность по операционной прибыли', 'рентабельность по ebitcac', 'ebitdac margin', 'ebitcac margin', 'operating margin'],
...
function extractSheetGid(urlOrId) {
  const s = String(urlOrId || '').trim();
  const match = s.match(/[?#&]gid=(\d+)/) || s.match(/#gid=(\d+)/);
  return match ? Number(match[1]) : null;
}

function findSheet(ss, requestedName, sourceRef, reportId) {
  const exactReportId = normText(reportId || '');
  const gid = extractSheetGid(sourceRef);
  if (exactReportId === 'opiu' && gid !== null && !isNaN(gid)) {
    try {
      const byGid = ss.getSheetById(gid);
      if (byGid) return byGid;
    } catch (e) {
      // Fall back to configured title search below.
    }
  }
  ...
  if (best && best.score >= 0.78) return best.sheet;
  if (gid !== null && !isNaN(gid)) {
    try {
      const byGid = ss.getSheetById(gid);
      if (byGid) return byGid;
    } catch (e) {
      // No sheet by gid; return null below.
    }
  }
  return null;
}
...
function writeSpeakerNote(note) {
  ...
  const blockId = String(note.block_id || note.chart_id || '').trim();
  const row = findSpeakerNoteRow(sheet, blockId);
  const values = [
    note.tab_id || '',
    blockId,
  ...
  return { ok: true, block_id: blockId, updated: row > 1 };
}

function findSpeakerNoteRow(sheet, blockId) {
  if (!blockId) return -1;
  const values = sheet.getDataRange().getValues();
  if (!values.length) return -1;
  const headers = values[0].map(h => normText(h));
  let idx = headers.indexOf('blockid');
  if (idx < 0) idx = headers.indexOf('chartid');
  if (idx < 0) idx = 1;
  for (let r = 1; r < values.length; r++) {
    if (String(values[r][idx] || '').trim() === blockId) return r + 1;
  }
  return -1;
}
...
// PATCH 2026-07-13: keep percentage metrics from matching money rows such as "Contribution margin, USD".
function metricScoreFor(metric, label) {
  const nLabel = normText(label);
  const originalLabel = String(label || '').toLowerCase();
  const pctMetrics = ['margin_pct', 'gross_margin_pct', 'op_margin_pct', 'np_margin_pct'];
  if (pctMetrics.indexOf(metric) >= 0) {
    if (/\b(usd|eur|rub|amount)\b|[$€₽]|руб|сумм/.test(originalLabel) || /\b(usd|eur|rub|amount)\b|руб|сумм/.test(nLabel)) return 0;
    if (metric === 'margin_pct' && !(originalLabel.indexOf('%') >= 0 || nLabel.indexOf('ratio') >= 0 || nLabel.indexOf('рентабель') >= 0)) return 0;
  }
  const rejects = METRIC_REJECT_TOKENS[metric] || [];
  if (rejects.some(t => nLabel.indexOf(normText(t)) >= 0)) return 0;
  const required = METRIC_REQUIRED_TOKENS[metric] || [];
  ...
  let score = metricScore(label, METRICS[metric]);
  const exactBoosts = {
    margin_pct: ['contribution margin ratio', 'рентабельность по маржинальному доходу'],
    op_margin_pct: ['рентабельность по операционной прибыли', 'рентабельность по ebitcac', 'ebitdac margin', 'ebitcac margin', 'operating margin'],
    np_margin_pct: ['рентабельность по чп', 'рентабельность по чп общая', 'рентабельность по чистой прибыли'],
    cash: ['cash at end of month', 'остаток денежных средств на конец', 'остаток дс на конец', 'конечный остаток'],
    total_ncf: ['total ncf monthly cash change', 'total ncf monthly cash', 'общий денежный поток']
