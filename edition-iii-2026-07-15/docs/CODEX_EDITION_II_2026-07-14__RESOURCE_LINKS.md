...
Финальная контрольная версия Edition II после локальной проверки 2026-07-14 должна публиковаться отдельной папкой:

`publish_bundle/edition-ii-2026-07-14/`

Ожидаемая GitHub Pages ссылка:

`https://epilipenko-byte.github.io/Codex-rocket-dashboard_presentation/edition-ii-2026-07-14/`

Старая версия `edition-ii-2026-07-13/` остается доступной как быстрый откат.

Ограничение: Apps Script web app URL пока не развернул action `get_core`; фронт работает через fallback `get_all`, а актуальный `apps_script_proxy.js` включен в пакет для последующего деплоя.
